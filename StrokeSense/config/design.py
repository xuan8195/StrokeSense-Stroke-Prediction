...
def load_lottie_file(filepath: str):
    ...
    try:
        # Since Streamlit Cloud runs from repository root, 
        # all paths should be relative to root directory
        if os.path.exists(filepath):
            with open(filepath, "r") as f:
                return json.load(f)
        
        # Try with different path combinations for flexibility
        possible_paths = [
            filepath,
            f"assets/{os.path.basename(filepath)}",
            f"StrokeSense/assets/{os.path.basename(filepath)}"
        ]
        ...
        # If file not found, return None and show warning
        st.warning(f"⚠️ Lottie animation file not found: {filepath}")
        return None
